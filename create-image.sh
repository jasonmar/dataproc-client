...
function await_shutdown() {
  ...
  for ((i = 0; i < 60; i++)); do
    ...
    sleep 30
    local STATUS=$(gcloud compute instances describe "${INSTANCE_NAME}" --zone="${ZONE}" 2>/dev/null | grep status | awk {'print $2'})
...
e=$?
[ $e -eq 0 ] || gsutil mb -p ${PROJECT} -c ${STORAGE_CLASS} gs://${BUCKET}
